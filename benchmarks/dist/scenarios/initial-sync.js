...
exports.runInitialSyncScenario = runInitialSyncScenario;
...
async function runInitialSyncScenario(options = {}) {
    const { threadCount = 500, runs = 3, resultsDir = path_1.default.join(__dirname, '../results') } = options;
    let gitSha = 'unknown';
    try {
    ...
    for (let run = 0; run < runs; run++) {
        ...
        const benchmarkDir = path_1.default.join(resultsDir, 'temp', `sync-${run}`);
        let electronApp = null;
        try {
            const startTime = Date.now();
            console.log('  Seeding database (simulating sync)...');
            await (0, seed_account_1.seedAccount)({ configDir: benchmarkDir, threadCount, messagesPerThread: 3 });
