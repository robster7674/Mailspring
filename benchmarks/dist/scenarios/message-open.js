...
exports.runMessageOpenScenario = runMessageOpenScenario;
...
async function runMessageOpenScenario(options = {}) {
    const { threadCount = 50, runs = 3, resultsDir = path_1.default.join(__dirname, '../results') } = options;
    let gitSha = 'unknown';
    try {
    ...
    for (let run = 0; run < runs; run++) {
        ...
        const benchmarkDir = path_1.default.join(resultsDir, 'temp', `msgopen-${run}`);
        let electronApp = null;
        try {
            console.log('  Seeding database...');
            await (0, seed_account_1.seedAccount)({ configDir: benchmarkDir, threadCount, messagesPerThread: 5 });
